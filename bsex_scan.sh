...
ExchIDsArr+=($(curl -sL api.bestchange.ru/info.zip | funzip - | awk -F';' '{print $1}' | sort | uniq ))
ExchIDsArrSize=$(printf "%s\n" "${ExchIDsArr[@]}"  | wc -l)

# Inform about working datasets
printf "Uniq IDs: %s\nCount Socks5: %s\n\n"  "${ExchIDsArrSize}"  "${Socks5ArrSize}"

for name in "${ExchIDsArr[@]}"
 do
    # Loop the request until we get a response from the server
    while true; do
        ProxyHost=$(selectRndProxy)
        #echo "DEBUG curl -sI "https://www.bestchange.ru/click.php?id=$name" -w '%{redirect_url}\n' -o /dev/null --socks5-hostname "${ProxyHost}""
